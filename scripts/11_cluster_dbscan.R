...
process_extremes_data <- function(input_dir, output_dir) {
  ...
  # Define subfolder names (used for output file naming)
  subfolder_name <- basename(input_dir)
  subsubfolder_name <- basename(dirname(input_dir))
  
  # Load a region raster (only used here for its CRS and grid properties)
  region.crs <- terra::rast(here::here("data", "output", "01_era5", "daily", "heat_index",
                                       "heat_index_daily_maximum_194001.nc"))
  
  # Create US land mask from state boundaries
  us.states <- st_as_sf(maps::map("state", plot = FALSE, fill = TRUE))
  us.states.rast <- st_rasterize(us.states %>% dplyr::select(1, geom))
  us.states.rast <- terra::rast(us.states.rast)
  us.states.rast <- us.states.rast / us.states.rast
  ...
  # Convert the raster mask to a polygon and buffer it (~1 degree, i.e., 111000 m)
  us_states_poly <- terra::as.polygons(us.states.rast, dissolve = TRUE)
  us_states_sf <- st_as_sf(us_states_poly)
  buffered_us_states_sf <- st_buffer(us_states_sf, dist = 111000)
  
  # Clip the buffered polygon to the extent of the raster and transform to WGS84
  rast_extent <- st_as_sfc(st_bbox(us.states.rast))
  final_buffered_polygon <- st_intersection(buffered_us_states_sf, rast_extent)
  final_buffered_polygon_fixed <- st_transform(final_buffered_polygon, crs = 4326)
  
  # Read and compile all CSV files from the input directory
  csv_files <- list.files(input_dir, pattern = "\\.csv$", full.names = TRUE)
  if (length(csv_files) == 0) {
    message("No CSV files found in ", input_dir, ". Skipping.")
    stop()
  ...
  # --- Precipitation Data: Ensure valid datetime column ---
  # If a column named "datetime" does not exist, look for a candidate column named with an 8-digit pattern.
  if (!"datetime" %in% names(compiled_data)) {
    potential <- names(compiled_data)[grepl("^\\d{8}$", names(compiled_data))]
    if (length(potential) >= 1) {
      names(compiled_data)[names(compiled_data) == potential[1]] <- "datetime"
    } else {
      stop("Unable to identify a valid datetime column in the compiled data.")
    }
  }
  # Remove any extra columns with names that match an 8-digit pattern (e.g. "20250416")
  compiled_data <- compiled_data %>% dplyr::select(-matches("^\\d{8}$"))
  
  # Select only the required columns (lat, long, datetime, observation, threshold)
  compiled_data_clean <- compiled_data %>% dplyr::select(lat, long, datetime, observation, threshold)
  
  # Convert the data frame into an sf object (using lat/long in EPSG:4326)
  points_sf <- st_as_sf(compiled_data_clean, coords = c("long", "lat"), crs = 4326)
  
  # Subset to points inside the buffered polygon
  inside_logical <- st_within(points_sf, final_buffered_polygon_fixed, sparse = FALSE)[, 1]
  points_inside <- points_sf[inside_logical, ]
  
  # Snap each point to the grid cell center from the region raster (without altering original lat/long in CSV)
  pts_coords <- st_coordinates(points_inside)
  cell_idx <- terra::cellFromXY(region.crs, pts_coords)
  grid_centers <- terra::xyFromCell(region.crs, cell_idx)
  points_inside$long <- grid_centers[, 1]
  ...
  # Normalize latitude and longitude: assign unique positions for each unique grid center
  lat.norm <- data.frame(values = sort(unique(extremes$lat)),
                         position = seq_along(unique(extremes$lat)))
  long.norm <- data.frame(values = sort(unique(extremes$long)),
                          position = seq_along(unique(extremes$long)))
  ...
  # Normalize the datetime variable:
  # Convert to numeric (seconds since epoch) then to hours.
  extremes.normalized$datetime.norm <- as.numeric(extremes.normalized$datetime) / 3600
  extremes.normalized$lat.norm <- as.numeric(extremes.normalized$lat.norm)
  extremes.normalized$long.norm <- as.numeric(extremes.normalized$long.norm)
  
  # Build a space–time cube (data frame) and perform clustering
  st.cube <- extremes.normalized
  gc()
  
  # Calculate kNN for DBSCAN (using k = 3; adjust neighbors as appropriate)
  kNN <- cluster.kNN.2(var.st.cube = st.cube %>% dplyr::select(lat.norm, long.norm, datetime.norm), neighbors = 3)
  gc()
  
  # Sort the cube by the normalized latitude, longitude, and datetime
  st.cube <- arrange(st.cube, lat.norm, long.norm, datetime.norm)
  
  # Perform DBSCAN clustering using the normalized columns
  clusters <- dbscan::dbscan(st.cube %>% dplyr::select(lat.norm, long.norm, datetime.norm),
                             eps = kNN$epsilon, minPts = kNN$mu)
  st.cube$cluster <- clusters$cluster
  st.cube$mu <- clusters$minPts
  st.cube$eps <- clusters$eps
  st.cube <- as_tibble(st.cube)
  
  # Write the clustered output as a CSV file
  output_file <- file.path(output_dir,
                           paste0(subfolder_name, "_", subsubfolder_name, "_clustered_extremes.csv"))
  write.csv(st.cube, output_file, row.names = FALSE)
  message("Output written to: ", output_file)
...
# Precipitation ----
input_dirs <- c(
  here::here("data", "output", "03_cluster", "01_extreme_points", "24hr1yr", "0.25")
)

# Define the common output directory
output_dir <- here::here("data", "output", "03_cluster", "02_cluster","24hr1yr")
...
heat_index_0.39_deg_day <- read.csv(here::here("data", "output", "03_cluster", "02_cluster","advisory", "heat_index_0.39_clustered_extremes.csv"))
heat_index_0.3075_deg_day <- read.csv(here::here("data", "output", "03_cluster", "02_cluster","advisory", "heat_index_0.3075_clustered_extremes.csv"))
precipitation_0.25_deg_hour <- read.csv(here::here("data", "output", "03_cluster", "02_cluster","24hr1yr", "0.25_24hr1yr_clustered_extremes.csv"))
